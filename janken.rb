...
class Player
    def hand
      ...
      while true
        ...
        puts "Saisissez un nombre compris entre 0 et 2: "
        puts " 0:pierre \n 1:Feuille \n 2:Ciseaux"

        input_hand= gets.chomp
        if input_hand == "O" || input_hand == "1" || input_hand == "2"
  ...
  class Janken
    def pon(player_hand, enemy_hand)
      ...
      janken = ["Pierre", "Feuille", "Ciseaux"]
      #「相手の手は#{相手の手}です。」と出力させます。
      puts "votre adversaire a choisi #{janken[enemy_hand]}"
      ...
      if player_hand == enemy_hand # Playerクラスの戻り値(player_hand)とEnemyクラスの戻り値(enemy_hand)の値が同じだった場合
        ...
        puts "Félicitation, vous etes le vainqueur"
        return false
        # もしも下記の組み合わせだった場合
